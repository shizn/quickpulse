...
{ 
  Write-Error "Issue encountered while installing functions core tools: ${InvokeCommandResult.Value[1].Message}"
  Break
}
Start-Sleep 10
...
$InvokeCommandResult = Invoke-AzVMRunCommand -ResourceGroupName $VMResourceGroup -VMName $ParticipantVM.Name -CommandId 'RunPowerShellScript' -ScriptPath './tools/ChromeSettings.ps1' -Parameter @{ChromeHomePage = "https://docs.microsoft.com/en-us/azure/functions/"}
If ($InvokeCommandResult.Status -eq "Succeeded")
{
  Write-Output "Installation succeded. ${InvokeCommandResult.Value[0].Message}"
}
...
{ 
  Write-Error "Issue encountered while configuring Chrome: ${InvokeCommandResult.Value[1].Message}"
  Break
}
Start-Sleep 10
...
$InvokeCommandResult = Invoke-AzVMRunCommand -ResourceGroupName $VMResourceGroup -VMName $ParticipantVM.Name -CommandId 'RunPowerShellScript' -ScriptPath './tools/InstallAzCLI.ps1'
If ($InvokeCommandResult.Status -eq "Succeeded")
{
  Write-Output "Installation succeded. ${InvokeCommandResult.Value[0].Message}"
}
...
{ 
  Write-Error "Issue encountered while installing Az CLI: ${InvokeCommandResult.Value[1].Message}"
  Break
}
Start-Sleep 20

# Install Pyhton
Write-Output "Installing Pyhton"
$InvokeCommandResult = Invoke-AzVMRunCommand -ResourceGroupName $VMResourceGroup -VMName $ParticipantVM.Name -CommandId 'RunPowerShellScript' -ScriptPath './tools/InstallPython.ps1' -Parameter @{Version = "3.7.0"}
If ($InvokeCommandResult.Status -eq "Succeeded")
{
  Write-Output "Installation succeded. ${InvokeCommandResult.Value[0].Message}"
}
...
{ 
  Write-Error "Issue encountered while installing Pyhton: ${InvokeCommandResult.Value[1].Message}"
  Break
}
Start-Sleep 20
